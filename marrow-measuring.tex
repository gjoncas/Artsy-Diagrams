...
\usepackage{musicography}	%for music note

\def\centerarc[#1](#2)(#3:#4:#5) {\draw[#1] ($(#2)+({#5*cos(#3)},{#5*sin(#3)})$) arc (#3:#4:#5);}

%from Venterus Mandey - Mellificium Mensionis: Or, the Marrow of Measuring (1727)
%https://archive.org/details/mellificiummensi00mand/page/n537/mode/2up
%with an explanation on pages 449-60
...
\begin{document}
	...
	\node at (3.1,2.2) {S};
	\node at (6.25,0) {E};
	\node at (5.15,2.15) {\musEighth}; %yes, this is actually in the original
	\end{tikzpicture}
